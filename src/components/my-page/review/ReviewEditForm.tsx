...
import { useReviewMutation } from '@/hooks/react-query/useReviewMutation';
...
export default function ReviewEditForm({
  ...
  const { create, update } = useReviewMutation(review.game_id, onClose, onSave);
  const handleSubmit = async () => {
    if (review.review_id === -1) {
      create.mutate({ content, rating });
    } else {
      update.mutate({ reviewId: review.review_id, content, rating });
    }
